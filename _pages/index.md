---
title: Seonho Lee
layout: home
permalink: /
description: Griffin Sunho (Seonho) Lee – Applied AI Researcher at Krafton AI
---

<h2 class="section-title">News</h2>

<ul class="news-list" id="newsListMain">
  <li class="news-item"><span class="news-date">Mar. 2026</span><span class="news-text">I begin to work as an Applied AI Researcher at <a href="https://www.krafton.com/en/" target="_blank">Krafton</a> <a href="https://www.krafton.ai/en/" target="_blank">AI</a>.</span></li>
  <li class="news-item"><span class="news-date">Jan. 2026</span><span class="news-text">Our <a href="/publications#c6"><b>What "Not" to Detect</b></a> paper got accepted to <a href="https://iclr.cc/Conferences/2026" target="_blank">ICLR 2026</a>!</span></li>
  <li class="news-item"><span class="news-date">Oct. 2025</span><span class="news-text">Two papers <a href="/publications#c4"><b>PartCATSeg</b></a> and <a href="/publications#c5"><b>3D-Aware VLM Finetuning</b></a> got selected as <a href="https://www.qualcomm.com/research/university-relations/innovation-fellowship/2025-south-korea" target="_blank"><b>QIFK 2025 Finalist</b></a>.</span></li>
  <li class="news-item"><span class="news-date">Aug. 2025</span><span class="news-text">Our <a href="/publications#c5"><b>3D-Aware VLM Finetuning</b></a> paper got accepted to <a href="https://2025.emnlp.org/" target="_blank"><b>EMNLP 2025 Findings</b></a>!</span></li>
  <li class="news-item"><span class="news-date">Jun. 2025</span><span class="news-text">I got ML engineering internship at <a href="https://www.snap.com" target="_blank">Snap Inc.</a> in Santa Monica, CA.</span></li>
  <li class="news-item"><span class="news-date">Jun. 2025</span><span class="news-text">I got awarded <a href="https://www.msit.go.kr/bbs/view.do?sCode=user&mPid=121&mId=311&bbsSeqNo=100&nttSeqNo=3179541" target="_blank"><b>Korean Presidential Science Scholarship for Graduate Students</b></a>.</span></li>
  <li class="news-item"><span class="news-date">May. 2025</span><span class="news-text">Our <a href="/publications#c3"><b>ScribbleDiff</b></a> paper got accepted to <a href="https://2025.ieeeicip.org/" target="_blank"><b>ICIP 2025</b></a>!</span></li>
  <li class="news-item"><span class="news-date">Feb. 2025</span><span class="news-text">Our <a href="/publications#c4"><b>PartCATSeg</b></a> paper got accepted to <a href="https://cvpr.thecvf.com/Conferences/2025" target="_blank"><b>CVPR 2025</b></a>!</span></li>
  <li class="news-item"><span class="news-date">Jan. 2025</span><span class="news-text">Our <a href="/publications#c2"><b>DreamCatalyst</b></a> paper got accepted to <a href="https://iclr.cc/Conferences/2025" target="_blank"><b>ICLR 2025</b></a>!</span></li>
</ul>

<div class="more-news-body" id="moreNewsBody">
  <ul class="news-list">
    <li class="news-item"><span class="news-date">Sep. 2024</span><span class="news-text">Our <a href="/publications#c1"><b>PartCLIPSeg</b></a> paper got accepted to <a href="https://neurips.cc/Conferences/2024" target="_blank"><b>NeurIPS 2024</b></a>!</span></li>
    <li class="news-item"><span class="news-date">Sep. 2023</span><span class="news-text">I got accepted to <a href="https://sites.google.com/view/cvml-kaist/home" target="_blank">CVML Lab</a> as a M.S. graduate student in KAIST AI.</span></li>
  </ul>
</div>
<button class="more-news-btn" id="moreNewsBtn" onclick="toggleMoreNews()">More News <i class="more-news-arrow">▾</i></button>

<h2 class="section-title">Selected Publications</h2>

<p style="font-size:0.85rem; color:var(--text-muted); margin-bottom:1.5rem;">*: equal contribution &nbsp;·&nbsp; †: corresponding author</p>

<ul class="pub-list">
  <li class="pub-entry">
    <img class="pub-thumb" src="/images/about/publications/negtome.png" alt="NegToMe">
    <div class="pub-body">
      <div class="pub-title"><a href="/publications#c6">What "Not" to Detect: Improving Object Detection under Negation via Reasoning and Token Merging</a></div>
      <div class="pub-authors">Inha Kang, Youngsun Lim, <b>Seonho Lee</b>, Jiho Choi, Junsuk Choi†, Hyunjung Shim†</div>
      <div class="pub-meta">
        <span class="venue-badge conf">ICLR 2026</span>
        <a class="pub-link" href="https://arxiv.org/abs/2510.13232" target="_blank">Paper</a>
      </div>
    </div>
  </li>
  <li class="pub-entry">
    <img class="pub-thumb" src="/images/about/publications/3d_aware_vlm.png" alt="3D-Aware VLM">
    <div class="pub-body">
      <div class="pub-title"><a href="/publications#c5">3D-Aware Vision-Language Models Fine-Tuning with Geometric Distillation</a></div>
      <div class="pub-authors"><b>Seonho Lee</b>*, Jiho Choi*, Inha Kang, Jiwook Kim, Junsung Park, Hyunjung Shim†</div>
      <div class="pub-meta">
        <span class="venue-badge conf">EMNLP 2025 Findings</span>
        <a class="pub-link" href="http://arxiv.org/abs/2506.09883" target="_blank">Paper</a>
        <a class="pub-link" href="https://github.com/kaist-cvml/3d-vlm-gd" target="_blank">Code</a>
      </div>
    </div>
  </li>
  <li class="pub-entry">
    <img class="pub-thumb" src="/images/about/publications/partcatseg.png" alt="PartCATSeg">
    <div class="pub-body">
      <div class="pub-title"><a href="/publications#c4">Fine-Grained Image-Text Correspondence with Cost Aggregation for Open-Vocabulary Part Segmentation</a></div>
      <div class="pub-authors">Jiho Choi*, <b>Seonho Lee</b>*, Seungho Lee, Minhyun Lee, Hyunjung Shim†</div>
      <div class="pub-meta">
        <span class="venue-badge conf">CVPR 2025</span>
        <a class="pub-link" href="http://arxiv.org/abs/2501.09688" target="_blank">Paper</a>
        <a class="pub-link" href="https://github.com/kaist-cvml/part-catseg" target="_blank">Code</a>
      </div>
    </div>
  </li>
  <li class="pub-entry">
    <img class="pub-thumb" src="/images/about/publications/dreamcatalyst.png" alt="DreamCatalyst">
    <div class="pub-body">
      <div class="pub-title"><a href="/publications#c2">DreamCatalyst: Fast and High-Quality 3D Editing via Controlling Editability and Identity Preservation</a></div>
      <div class="pub-authors">Jiwook Kim*, <b>Seonho Lee</b>*, Jaeyo Shin, Jiho Choi, Hyunjung Shim†</div>
      <div class="pub-meta">
        <span class="venue-badge conf">ICLR 2025</span>
        <a class="pub-link" href="https://arxiv.org/abs/2407.11394" target="_blank">Paper</a>
        <a class="pub-link" href="https://dream-catalyst.github.io/" target="_blank">Project</a>
        <a class="pub-link" href="https://github.com/kaist-cvml-lab/DreamCatalyst" target="_blank">Code</a>
      </div>
    </div>
  </li>
  <li class="pub-entry">
    <img class="pub-thumb" src="/images/about/publications/partclipseg.png" alt="PartCLIPSeg">
    <div class="pub-body">
      <div class="pub-title"><a href="/publications#c1">Understanding Multi-Granularity for Open-Vocabulary Part Segmentation</a></div>
      <div class="pub-authors">Jiho Choi*, <b>Seonho Lee</b>*, Seungho Lee, Minhyun Lee, Hyunjung Shim†</div>
      <div class="pub-meta">
        <span class="venue-badge conf">NeurIPS 2024</span>
        <a class="pub-link" href="https://arxiv.org/abs/2406.11384" target="_blank">Paper</a>
        <a class="pub-link" href="https://github.com/kaist-cvml-lab/part-clipseg" target="_blank">Code</a>
      </div>
    </div>
  </li>
</ul>

<a href="/publications" class="btn-all">All Publications →</a>

<h2 class="section-title">Work Experience</h2>

<ul class="exp-list">
  <li class="exp-entry">
    <img class="exp-logo" src="/images/about/companies/krafton.png" alt="Krafton">
    <div class="exp-body">
      <div class="exp-title"><a href="https://www.krafton.ai/en/" target="_blank">Krafton AI</a> — Applied AI Researcher</div>
      <div class="exp-sub">Mar. 2026 – Present · Seoul, Korea</div>
      <ul class="exp-bullets">
        <li>Working in AI Art Gen Team at Krafton AI</li>
      </ul>
    </div>
  </li>
  <li class="exp-entry">
    <img class="exp-logo" src="/images/about/companies/snap.png" alt="Snap">
    <div class="exp-body">
      <div class="exp-title"><a href="https://www.snap.com" target="_blank">Snap Inc.</a> — ML Engineer Intern, Generative ML (VideoCraft)</div>
      <div class="exp-sub">Jun. 2025 – Sep. 2025 · Santa Monica, CA</div>
      <ul class="exp-bullets">
        <li>Led cross-reference dataset preprocessing pipeline for personalized video generation</li>
        <li>Developed multi-subject adapter architecture for <em>VideoAlchemist 2.0</em> (foundation for <a href="https://arxiv.org/abs/2512.10943" target="_blank">AlcheMinT</a>)</li>
      </ul>
    </div>
  </li>
</ul>

<h2 class="section-title">Selected Projects</h2>

<div class="projects-grid">
  <a class="project-card has-thumb" href="https://krafton.ai/blog/posts/2026-04-02-raon_vision_encoder/raon-visionencoder-en.html" target="_blank">
    <div class="project-card-inner">
      <div class="project-info">
        <div class="project-name">Raon-VisionEncoder</div>
        <div class="project-desc">SigLIP2-class vision encoder trained entirely on open data · Krafton AI</div>
        <div class="project-tags"><span>Vision Encoder</span><span>VLM</span></div>
      </div>
    </div>
  </a>
  <a class="project-card has-thumb" href="/project/shrack">
    <div class="project-card-inner">
      <img class="project-thumb" src="https://vip2.loli.io/2023/09/02/AZ4Wa3YGPjgmbR8.png" alt="SHrack" style="object-fit:contain;background:#fff;">
      <div class="project-info">
        <div class="project-name">SHrack</div>
        <div class="project-desc">Mobile web service for real-time exercise posture detection and count tracking</div>
        <div class="project-tags"><span>MobileNet</span><span>Computer Vision</span></div>
      </div>
    </div>
  </a>
  <a class="project-card has-thumb" href="/project/recjoon">
    <div class="project-card-inner">
      <img class="project-thumb" src="https://cdn.jsdelivr.net/gh/Glanceyes/Image-Repository/2022/06/13/20220613_1655087579.png" alt="RECJOON" style="object-fit:contain;background:#fff;">
      <div class="project-info">
        <div class="project-name">RECJOON</div>
        <div class="project-desc">Personalized algorithm problem recommendation system for Baekjoon Online Judge</div>
        <div class="project-tags"><span>Recommendation</span><span>Deep Learning</span></div>
      </div>
    </div>
  </a>
</div>

<a href="/projects" class="btn-all">All Projects →</a>

<h2 class="section-title">Education</h2>

<ul class="edu-list">
  <li class="edu-entry">
    <div class="edu-school"><a href="https://www.kaist.ac.kr/en/" target="_blank">KAIST</a></div>
    <div class="edu-degree">M.S. in Artificial Intelligence · Mar. 2024 – Feb. 2026</div>
    <div class="edu-detail">GPA: 4.20 / 4.3 · Advised by <a href="https://scholar.google.com/citations?user=KB5XZGIAAAAJ" target="_blank">Prof. Hyunjung Shim</a></div>
  </li>
  <li class="edu-entry">
    <div class="edu-school"><a href="https://www.sogang.ac.kr/en/home" target="_blank">Sogang University</a></div>
    <div class="edu-degree">B.S. in Computer Science and Engineering · Mar. 2017 – Feb. 2024</div>
    <div class="edu-detail">GPA: 4.12 / 4.3 · Summa Cum Laude</div>
  </li>
</ul>

<h2 class="section-title">Honors &amp; Awards</h2>

<ul class="honor-list">
  <li class="honor-entry">
    <div class="honor-body">
      <div class="honor-title">Grand Prize, <a href="http://www.ipiu.or.kr/" target="_blank">IPIU 2026</a></div>
      <div class="honor-sub">Paper: <a href="/publications#c6">What "Not" to Detect</a> · Feb. 2026 · Inha Kang, <b>Seonho Lee</b>, Jiho Choi</div>
    </div>
  </li>
  <li class="honor-entry">
    <div class="honor-body">
      <div class="honor-title"><a href="https://www.qualcomm.com/research/university-relations/innovation-fellowship/2025-south-korea" target="_blank">Qualcomm Innovation Fellowship Korea (QIFK) 2025 Finalist</a></div>
      <div class="honor-sub">Selected by Qualcomm AI Research · Oct. 2025 · Papers: <a href="/publications#c4">PartCATSeg</a>, <a href="/publications#c5">3D-Aware VLM Finetuning</a></div>
    </div>
  </li>
  <li class="honor-entry">
    <div class="honor-body">
      <div class="honor-title"><a href="https://www.msit.go.kr/bbs/view.do?sCode=user&mPid=121&mId=311&bbsSeqNo=100&nttSeqNo=3179541" target="_blank">Korean Presidential Science Scholarship for Graduate Students</a></div>
      <div class="honor-sub">Awarded by the President of Korea · Jun. 2025</div>
    </div>
  </li>
  <li class="honor-entry">
    <div class="honor-body">
      <div class="honor-title">2nd Place, Open Vocabulary Part Segmentation Challenge @ CVPR 2024</div>
      <div class="honor-sub">Track 1 &amp; 2 · <a href="https://vplow.github.io/vplow_4th.html" target="_blank">4th Workshop on Open World Vision (VPLOW)</a></div>
    </div>
  </li>
  <li class="honor-entry">
    <div class="honor-body">
      <div class="honor-title">Excellence Award, <a href="https://dataen.ai/challenge/history" target="_blank">2023 POSTECH OIBC Challenge</a></div>
      <div class="honor-sub">3rd Place (3/120) · Solar Power Generation Forecasting · Dec. 2023</div>
    </div>
  </li>
  <li class="honor-entry">
    <div class="honor-body">
      <div class="honor-title">2022 ICPC Asia Korea Regional Contest</div>
      <div class="honor-sub">48th in Korea · 62nd in Preliminary</div>
    </div>
  </li>
  <li class="honor-entry">
    <div class="honor-body">
      <div class="honor-title">Korea National Science and Technology Scholarship</div>
      <div class="honor-sub">Spring 2019, Fall 2022, Spring 2023, Fall 2023</div>
    </div>
  </li>
  <li class="honor-entry">
    <div class="honor-body">
      <div class="honor-title">Dean's List, Sogang University</div>
      <div class="honor-sub">Top 1%: Spring 2018, Spring 2019, Fall 2022 · Top 5%: Fall 2018</div>
    </div>
  </li>
</ul>

<h2 class="section-title">Academic Activities</h2>

<ul class="honor-list">
  <li class="honor-entry">
    <div class="honor-body">
      <div class="honor-title">Reviewer</div>
      <div class="honor-sub"><b>2026</b>: CVPRW 2026, 3DV 2026</div>
    </div>
  </li>
</ul>
